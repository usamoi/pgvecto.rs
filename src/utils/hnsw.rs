use super::unsafe_once::UnsafeOnce;
...
use crate::utils::visited::Visited;
...
type Vertex = PBox<[RwLock<PArray<(Tcalar, usize)>>]>;
...
impl HnswImpl {
    pub fn new(
        ...
        let ptr = PBox::new(
            Root {
                vertexs: unsafe { PBox::new_zeroed_slice(capacity, allocator)?.assume_init() },
    ...
    pub fn search(
        ...
        anyhow::ensure!(vector.len() == self.dims as usize);
        let entry = self.root.entry.read().clone();
        ...
        Ok(result
            .iter()
            .map(|&(score, u)| (score.0, self.vectors.get_data(u)))
            .collect::<Vec<_>>())
    }
    ...
    fn _insert(&self, visited: &mut Visited, insert: usize) -> anyhow::Result<()> {
        ...
        let Some(mut u) = entry else {
            let vertex = {
                let mut vertex = PBox::new_uninit_slice(1 + levels as usize, self.allocator)?;
                for i in 0..=levels {
                    let array = PArray::new(1 + size_of_a_layer(self.m, i), self.allocator)?;
        ...
        let vertex = {
            let mut vertex = PBox::new_uninit_slice(1 + levels as usize, self.allocator)?;
            for i in 0..=levels {
                let mut array = PArray::new(1 + size_of_a_layer(self.m, i), self.allocator)?;
    ...
    fn _dist0(&self, u: usize, target: &[Scalar]) -> Tcalar {
        let u = self.vectors.get_vector(u).as_ref();
        Tcalar(self.distance.distance(u, target))
    }
    fn _dist1(&self, u: usize, v: usize) -> Tcalar {
        let u = self.vectors.get_vector(u).as_ref();
        let v = self.vectors.get_vector(v).as_ref();
